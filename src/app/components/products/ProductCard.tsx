...
export default function ProductCard({ item }: { item: any }) {
  ...
  return (
    <Link href={`/products/${item?.id}`} className="block text-center group">
      <div
        key={item?.id}
        className="aspect-square relative mb-4 overflow-hidden"
      >
        {/* <Image
          src={item.image}
        ...
        {item.images && item?.images?.length > 1 ? (
          ...
          <Image
            src={item?.image || "eror"}
            alt={item?.product_name}
            className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500 bg-gray-200"
            height={300}
